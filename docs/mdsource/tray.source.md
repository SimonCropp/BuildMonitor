 ...
 * two marks: the logo of the service that ran the build, badged in its corner, and the bare logo of the service hosting its source. The badge is a play, or a red cross where the run it opens broke, and it is what tells the two apart where they are the same company: GitHub Actions is the octocat badged and the repository beside it is the octocat alone. A host nothing here has a mark for, most self hosted Git, gets none, and nor does a provider that reports no repository
