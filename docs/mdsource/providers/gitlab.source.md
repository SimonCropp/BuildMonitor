...
A token with `read_api` and not `api`, pasted or from a sign in, offers neither Retry nor Cancel, and the connection shows as watch only in [Connections](../connections.md). Nor does a project where the user is only a Reporter, unless the user is an administrator.
