...
/// </summary>
sealed record ProjectedRows(ImmutableArray<PipelineBuilds> Pipelines, ImmutableArray<ConnectionState> Connections, ImmutableHashSet<string> OpenGroups, string Search, ImmutableArray<string> GroupPrefixes, bool GroupByOrg, ImmutableArray<Row> Rows)
{
    public bool IsFor(SessionState state, ImmutableArray<PipelineBuilds> pipelines) =>
        ...
        Search == state.Search &&
        // Saved prefixes, or grouping by owner, change which builds share a group, and nothing else a projection reads
